...
CUDA_VISIBLE_DEVICES=$device OMP_NUM_THREADS=$num_threads python -m joeynmt translate $configs/$model_name.yaml < $data/test.tokenized.de-it.$src > $translations_sub/test.word.$model_name.$trg

# undo tokenization

cat $translations_sub/test.word.$model_name.$trg | $MOSES/tokenizer/detokenizer.perl -l $trg > $translations_sub/test.$model_name.$trg
...
################################################################################################
# evaluate bpe5000 model

# measure time
SECONDS=0

model_name=transformer_bpe5000_config
